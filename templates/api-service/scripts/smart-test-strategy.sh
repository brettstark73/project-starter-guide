#!/bin/bash
# Smart Test Strategy - API Service Template
set -e
...
# Project-specific high-risk patterns (API service)
HIGH_RISK_FILES=$(git diff --name-only HEAD~1..HEAD 2>/dev/null | grep -E "(src/routes|src/controllers|src/middleware|src/auth|prisma/schema)" || true)
API_FILES=$(git diff --name-only HEAD~1..HEAD 2>/dev/null | grep -E "src/(routes|controllers|middleware)" || true)
CONFIG_FILES=$(git diff --name-only HEAD~1..HEAD 2>/dev/null | grep -E "(package\.json|\.env|config|docker)" || true)
...
echo ""
echo "💡 Tip: Run 'npm test && npm run test:integration' locally for full validation"
