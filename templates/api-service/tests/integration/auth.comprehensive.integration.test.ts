...
describe("Comprehensive Authentication Integration Tests (Real Database)", () => {
  ...
  describe("Complete Authentication Flow with Enhanced Database Simulation", () => {
    it("should register user, verify JWT/hashing, and login successfully", async () => {
      ...
      expect(updatedUser!.lastLogin!.getTime()).toBeGreaterThan(userInDb!.createdAt.getTime());
    });
    ...
    it("should maintain data integrity across operations", async () => {
      ...
      const users = [
        { name: "User One", email: generateUniqueEmail('user1'), password: "Password1!" },
        { name: "User Two", email: generateUniqueEmail('user2'), password: "Password2!" },
        { name: "User Three", email: generateUniqueEmail('user3'), password: "Password3!" },
      ];
